...
(function () {
    ...
    style.textContent = [
        '#aqs-news-ticker-bar{display:none;overflow:hidden;height:36px;line-height:36px;font-size:.82rem;font-weight:500;position:fixed;bottom:0;left:0;right:0;z-index:9990;}',
        ...
        '#aqs-ticker-track{display:inline-block;white-space:nowrap;padding-left:100%;animation:aqsTickerScroll 30s linear infinite;}',
        ...
        '#aqs-countdown-bar{display:none;text-align:center;padding:9px 20px;font-size:.88rem;position:fixed;bottom:36px;left:0;right:0;z-index:9989;}',
        ...
        '@media(max-width:480px){#aqs-news-ticker-bar{height:30px;line-height:30px;font-size:.75rem;}#aqs-news-ticker-bar .aqs-ticker-label{font-size:.7rem;padding:0 10px;}#aqs-countdown-bar{font-size:.8rem;padding:7px 12px;}.aqs-cd-block{font-size:.85rem;padding:2px 7px;min-width:32px;}}'
    ].join('');
    ...
    function buildBars() {
        ...
        /* Append at end of body — bars are position:fixed;bottom:0 so DOM position doesn't matter */
        document.body.appendChild(tickerBar);
        document.body.appendChild(cdBar);
    }


    /* ── Add body bottom padding so content isn't hidden behind fixed bars ── */
    var _paddingAdded = false;
    function _addBodyPadding() {
        if (_paddingAdded) return;
        _paddingAdded = true;
        var ticker = document.getElementById('aqs-news-ticker-bar');
        var cd     = document.getElementById('aqs-countdown-bar');
        var totalH = 0;
        if (ticker && ticker.style.display !== 'none') totalH += 36;
        if (cd     && cd.style.display     !== 'none') totalH += 44;
        if (totalH > 0) {
            var existing = parseInt(window.getComputedStyle(document.body).paddingBottom) || 0;
            document.body.style.paddingBottom = Math.max(existing, totalH + 8) + 'px';
        }
    }
    ...
    function startTicker(text, speed, label, bg, color) {
        ...
        /* Push page content up so nothing hides behind fixed bottom bar */
        _addBodyPadding();
    }
    ...
    function loadNotifSettings() {
        if (typeof window.aqsAjax !== 'function') {
            /* Retry once more after a short delay in case aqsAjax loaded late */
            setTimeout(function() {
