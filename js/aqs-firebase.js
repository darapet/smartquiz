...
async function actionChCreate(data) {
    ...
    /* League mode needs far fewer questions: qpr per shared round × numRounds.
       Standard mode needs numPlayers × qpr per round × numRounds. */
    var numQ = leagueMode ? (qpr * numRounds) : (numPlayers * qpr * numRounds);
    var challengeData = {
        code:                code,
        host_uid:            hostUid,
        title:               title,
...
async function actionChStart(data) {
    ...
    if (leagueMode) {
        /* League mode: all players share the same question pool.
           Each league round (game round) uses qpr questions indexed at
           (q_in_round-1) + (game_round-1)*qpr, so we need qpr*(numPlayers-1)
           questions per player. All players get the identical sequence. */
        var leagueRounds = Math.max(1, numPlayers - 1);
        var leaguePerPlayer = qpr * leagueRounds;
        for (var p = 0; p < numPlayers; p++) {
            var playerQs = [];
            for (var qi = 0; qi < leaguePerPlayer; qi++) {
                playerQs.push(questions[qi % qLen]);
            }
...
async function actionSaveSettings(data) {
    ...
    var payload = {};
    var allowed = [
        'groq_api_key','groq_model','bg_music_url',
